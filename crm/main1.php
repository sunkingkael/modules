...
define( 'BNTM_CRM_SCHEMA_VERSION', '1.2.0' );
...
function crm_get_fixed_pipelines() {
    return [
        1 => [
            'id'   => 1,
            'slug' => 'subscription',
...
function crm_get_pipeline( $pipeline_id ) {
    $pipeline_id = absint( $pipeline_id );
    $pipelines   = crm_get_fixed_pipelines();
    return $pipelines[ $pipeline_id ] ?? $pipelines[ crm_get_default_pipeline_id() ];
}

function crm_get_pipeline_name( $pipeline_id ) {
    $pipeline = crm_get_pipeline( $pipeline_id );
    return $pipeline['name'] ?? 'Pipeline';
}

function crm_get_active_pipeline_id() {
    $pipeline_id = absint( $_GET['pipeline'] ?? $_POST['pipeline_id'] ?? 0 );
    $pipelines   = crm_get_fixed_pipelines();

    if ( ! isset( $pipelines[ $pipeline_id ] ) ) {
        $pipeline_id = crm_get_default_pipeline_id();
    }

    return $pipeline_id;
}

function crm_get_pipeline_tab_url( $tab, $pipeline_id, $view = '', $record_id = 0 ) {
    $pipelines   = crm_get_fixed_pipelines();
    $pipeline_id = absint( $pipeline_id );
    if ( ! isset( $pipelines[ $pipeline_id ] ) ) {
        $pipeline_id = crm_get_default_pipeline_id();
    ...
    if ( $view !== '' ) {
        $args['view'] = sanitize_text_field( $view );
    }
    ...
    return esc_url( add_query_arg( $args, remove_query_arg( [ 'pipeline', 'view', 'id' ] ) ) );
}

function crm_get_pipeline_stage_blueprints() {
    return [
...
function crm_get_default_pipeline_product_blueprints() {
    return [
        1 => [
            [ 'name' => 'BNTM HUB',      'sort_order' => 1 ],
            [ 'name' => 'Spree Rewards', 'sort_order' => 2 ],
        ],
        2 => [
            [ 'name' => 'Bulk XAPS',               'sort_order' => 1 ],
            [ 'name' => 'Custom Spree',            'sort_order' => 2 ],
            [ 'name' => 'Custom BNTM HUB Modules', 'sort_order' => 3 ],
            [ 'name' => 'System Customization',     'sort_order' => 4 ],
            [ 'name' => 'Custom Web Dev',           'sort_order' => 5 ],
            [ 'name' => 'Custom App Dev',           'sort_order' => 6 ],
        ],
    ];
}

function crm_get_known_business_ids() {
    global $wpdb;
    ...
    foreach ( $tables as $table ) {
        $rows = $wpdb->get_col( "SELECT DISTINCT business_id FROM {$wpdb->prefix}{$table} WHERE business_id > 0" );
        foreach ( (array) $rows as $business_id ) {
            $business_ids[] = absint( $business_id );
        }
    }

    $business_ids = array_values( array_unique( array_filter( $business_ids ) ) );

    if ( empty( $business_ids ) ) {
        $business_ids[] = crm_get_current_business_scope_id();
    }
...
function crm_get_pipeline_stages_for_business( $business_id, $pipeline_id ) {
    global $wpdb;

    return $wpdb->get_results( $wpdb->prepare(
        "SELECT * FROM {$wpdb->prefix}crm_pipeline_stages
         WHERE business_id = %d AND pipeline_id = %d AND status = 'active'
...
function crm_get_pipeline_products_for_business( $business_id, $pipeline_id ) {
    global $wpdb;

    return $wpdb->get_results( $wpdb->prepare(
        "SELECT * FROM {$wpdb->prefix}crm_products
         WHERE business_id = %d AND pipeline_id = %d AND status = 'active'
         ORDER BY sort_order ASC",
        absint( $business_id ),
        absint( $pipeline_id )
...
function crm_get_all_pipeline_stages_for_business( $business_id ) {
    global $wpdb;

    return $wpdb->get_results( $wpdb->prepare(
        "SELECT * FROM {$wpdb->prefix}crm_pipeline_stages
         WHERE business_id = %d AND status = 'active'
         ORDER BY pipeline_id ASC, sort_order ASC",
        absint( $business_id )
    ) );
}

function crm_get_all_pipeline_products_for_business( $business_id ) {
    global $wpdb;
...
function crm_get_pipeline_dashboard_summary_for_business( $business_id, $pipeline_id ) {
    global $wpdb;
    ...
    if ( $business_id <= 0 || ! isset( $pipelines[ $pipeline_id ] ) ) {
        return [
            'pipeline_id'      => $pipeline_id,
            'pipeline_name'    => crm_get_pipeline_name( $pipeline_id ),
            'total_open_deals' => 0,
            'pipeline_value'   => 0,
            'stage_breakdown'  => [],
        ];
    ...
    $total_open_deals = (int) $wpdb->get_var( $wpdb->prepare(
        "SELECT COUNT(*) FROM {$wpdb->prefix}crm_deals
         WHERE business_id = %d AND pipeline_id = %d AND status = 'open'",
        $business_id, $pipeline_id
    ) );
...
function crm_get_pipeline_default_ids_for_business( $business_id ) {
    $business_id = absint( $business_id );
    $defaults = [];

    foreach ( crm_get_fixed_pipelines() as $pipeline_id => $pipeline ) {
        $defaults[ $pipeline_id ] = [
            'stage_id'   => crm_get_first_pipeline_stage_id( $business_id, $pipeline_id ),
            'product_id' => crm_get_first_pipeline_product_id( $business_id, $pipeline_id ),
        ];
    }
...
function crm_get_first_pipeline_stage_id( $business_id, $pipeline_id ) {
    global $wpdb;
...
function crm_get_first_pipeline_product_id( $business_id, $pipeline_id ) {
    global $wpdb;

    return (int) $wpdb->get_var( $wpdb->prepare(
        "SELECT id FROM {$wpdb->prefix}crm_products
         WHERE business_id = %d AND pipeline_id = %d AND status = 'active'
         ORDER BY sort_order ASC, id ASC
         LIMIT 1",
        absint( $business_id ),
...
function crm_seed_default_pipeline_stages_for_business( $business_id, $pipeline_id ) {
    global $wpdb;

    $business_id = absint( $business_id );
    $pipeline_id = absint( $pipeline_id );
    if ( $business_id <= 0 || ! isset( crm_get_fixed_pipelines()[ $pipeline_id ] ) ) {
        return;
    }
    ...
    foreach ( crm_get_pipeline_stage_blueprints() as $stage ) {
...
function crm_seed_default_pipeline_products_for_business( $business_id, $pipeline_id ) {
    global $wpdb;
    ...
    $blueprints = crm_get_default_pipeline_product_blueprints();
    $blueprints = $blueprints[ $pipeline_id ] ?? [];
    if ( empty( $blueprints ) ) {
        return;
    }

    $existing_names = $wpdb->get_col( $wpdb->prepare(
        "SELECT name FROM {$wpdb->prefix}crm_products
         WHERE business_id = %d AND pipeline_id = %d AND status = 'active'",
        $business_id,
        $pipeline_id
    ) );
    ...
    $max_order = (int) $wpdb->get_var( $wpdb->prepare(
        "SELECT COALESCE(MAX(sort_order), 0) FROM {$wpdb->prefix}crm_products
         WHERE business_id = %d AND pipeline_id = %d AND status = 'active'",
        $business_id,
        $pipeline_id
    ) );

    foreach ( $blueprints as $product ) {
        if ( in_array( $product['name'], $existing_names, true ) ) {
            continue;
        }

        $sort_order = $max_order > 0 ? $max_order + 1 : (int) $product['sort_order'];
        $wpdb->insert(
            $wpdb->prefix . 'crm_products',
            [
                'rand_id'     => bntm_rand_id(),
                'business_id' => $business_id,
                'pipeline_id' => $pipeline_id,
...
function crm_backfill_deal_products_for_business( $business_id ) {
    global $wpdb;
    ...
    foreach ( crm_get_fixed_pipelines() as $pipeline_id => $pipeline ) {
        ...
        $wpdb->query( $wpdb->prepare(
            "UPDATE {$wpdb->prefix}crm_deals
             SET product_id = %d
             WHERE business_id = %d
               AND pipeline_id = %d
               AND (product_id = 0 OR product_id IS NULL)",
            $product_id,
            $business_id,
            $pipeline_id
        ) );
...
function crm_sync_default_pipeline_data_for_business( $business_id ) {
    $business_id = absint( $business_id );
    if ( $business_id <= 0 ) {
        return;
    }
    ...
    foreach ( crm_get_fixed_pipelines() as $pipeline_id => $pipeline ) {
        crm_seed_default_pipeline_stages_for_business( $business_id, $pipeline_id );
        crm_seed_default_pipeline_products_for_business( $business_id, $pipeline_id );
    }

    crm_backfill_deal_products_for_business( $business_id );
}

function crm_sync_default_pipeline_data_for_all_businesses() {
    foreach ( crm_get_known_business_ids() as $business_id ) {
        crm_sync_default_pipeline_data_for_business( $business_id );
    }
}
...
function bntm_crm_get_tables() {
    ...
    return [
        ...
        'crm_products' => "CREATE TABLE {$prefix}crm_products (
            id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
            rand_id VARCHAR(20) UNIQUE NOT NULL,
            business_id BIGINT UNSIGNED NOT NULL DEFAULT 0,
            pipeline_id BIGINT UNSIGNED NOT NULL DEFAULT 1,
...
function bntm_crm_create_tables() {
    ...
    crm_sync_default_pipeline_data_for_all_businesses();
...
// Settings — Products
add_action( 'wp_ajax_crm_save_product',          'bntm_ajax_crm_save_product' );
add_action( 'wp_ajax_crm_delete_product',        'bntm_ajax_crm_delete_product' );
add_action( 'wp_ajax_crm_reorder_products',      'bntm_ajax_crm_reorder_products' );
...
function bntm_shortcode_crm() {
    ...
    $active_pipeline_id = crm_get_active_pipeline_id();
    $crm_users    = get_users( [ 'fields' => [ 'ID', 'display_name' ] ] );

    crm_sync_default_pipeline_data_for_business( $business_id );
    $pipeline_defaults    = crm_get_pipeline_default_ids_for_business( $business_id );
    $all_pipeline_stages  = crm_get_all_pipeline_stages_for_business( $business_id );
    $all_pipeline_products = crm_get_all_pipeline_products_for_business( $business_id );
    ...
    var crm_active_pipeline_id = <?php echo intval( $active_pipeline_id ); ?>;
    var crm_pipelines = <?php echo wp_json_encode( array_values( crm_get_fixed_pipelines() ) ); ?>;
    var crm_pipeline_defaults = <?php echo wp_json_encode( $pipeline_defaults ); ?>;
    var crm_pipeline_stages = <?php echo wp_json_encode( array_map( function( $stage ) {
        return [
            'id' => (int) $stage->id,
    ...
    var crm_pipeline_products = <?php echo wp_json_encode( array_map( function( $product ) {
        return [
            'id' => (int) $product->id,
            'pipeline_id' => (int) $product->pipeline_id,
            'name' => $product->name,
            'sort_order' => (int) $product->sort_order,
        ];
    }, $all_pipeline_products ) ); ?>;
    ...
    .crm-pipeline-switcher-list {
        display: flex;
        gap: 8px;
        flex-wrap: wrap;
    }
    .crm-pipeline-switcher-item {
        display: inline-flex;
        align-items: center;
        padding: 8px 12px;
        border-radius: 999px;
    ...
    .crm-pipeline-switcher-item:hover {
        border-color: var(--bntm-primary);
        color: var(--bntm-primary);
        transform: translateY(-1px);
    }
    .crm-pipeline-switcher-item.active {
        background: var(--bntm-primary);
        border-color: var(--bntm-primary);
        color: #fff;
    }
    ...
    /* ── Settings: sortable products ── */
    .crm-product-list { list-style: none; margin: 0; padding: 0; }
    .crm-product-item {
        display: flex;
        align-items: center;
        gap: 10px;
    ...
    .crm-product-item.drag-over { background: #f0fdf4; border-color: #16a34a; }
    .crm-product-item .crm-product-drag-handle { color: #d1d5db; cursor: grab; flex-shrink: 0; }
    .crm-product-item .crm-product-name {
        flex: 1;
        font-size: 14px;
        font-weight: 500;
        color: #111827;
    ...
    (function() {
        ...
        window.crmPost = function(action, data, onSuccess, onError) {
            ...
            if (typeof data.pipeline_id === 'undefined' && typeof window.crm_active_pipeline_id !== 'undefined') {
                data.pipeline_id = window.crm_active_pipeline_id;
            }
...
function crm_render_record_detail_page( $object_type, $detail, $back_url, $edit_label = 'Edit Record' ) {
    ...
    if ( $object_type === 'contact' ) {
        ...
        if ( ! empty( $detail['related_deals'] ) ) {
            ...
            foreach ( $detail['related_deals'] as $deal ) {
                $items .= '<div class="crm-detail-association-item">'
                    . '<a class="crm-detail-inline-link crm-detail-association-title" href="' . crm_get_pipeline_tab_url( 'deals', $deal['pipeline_id'] ?? crm_get_default_pipeline_id(), 'deal', $deal['id'] ) . '">' . esc_html( $deal['name'] ) . '</a>'
                    . '<div class="crm-detail-association-meta">' . crm_format_currency( $deal['amount'] ) . ' · ' . esc_html( $deal['product_name'] ?: 'No product' ) . ' · ' . esc_html( $deal['stage_name'] ?: 'No stage' ) . '</div>'
                    . '</div>';
            }
        ...
        $fields = [
            ...
            crm_render_detail_field( 'Pipeline', crm_get_pipeline_name( $detail['pipeline_id'] ?? crm_get_default_pipeline_id() ), false ),
            crm_render_detail_field( 'Product', $detail['product_name'] ?? '', false, [ 'editable' => true, 'object_type' => 'deal', 'field_key' => 'product' ] ),
...
function crm_dashboard_tab( $business_id ) {
    ...
    $pipeline_summaries = [];
    foreach ( crm_get_fixed_pipelines() as $pipeline_id => $pipeline ) {
        $pipeline_summaries[ $pipeline_id ] = crm_get_pipeline_dashboard_summary_for_business( $business_id, $pipeline_id );
    }
    ...
    <div class="crm-two-col">
        ...
        <div>
            ...
            <div class="crm-pipeline-summary-grid">
                <?php foreach ( $pipeline_summaries as $summary ) : ?>
                <div class="bntm-form-section crm-pipeline-summary-card">
                    <div class="crm-pipeline-summary-head">
                        <div>
                            <h3 style="margin:0;padding:0;border:none;"><?php echo esc_html( $summary['pipeline_name'] ); ?></h3>
                            <p style="font-size:13px;color:#6b7280;margin:4px 0 0;">
                                Pipeline-specific stage breakdown.
                            </p>
                        </div>
                        <a class="bntm-btn-secondary bntm-btn-small"
                           href="<?php echo crm_get_pipeline_tab_url( 'deals', $summary['pipeline_id'] ); ?>">
                            Open Deals
                        </a>
                    </div>

                    <div class="crm-pipeline-summary-metrics">
                        <div class="crm-pipeline-summary-metric">
                            <span>Open Deals</span>
                            <strong><?php echo number_format( $summary['total_open_deals'] ); ?></strong>
                        </div>
                        <div class="crm-pipeline-summary-metric">
                            <span>Open Value</span>
                            <strong><?php echo crm_format_currency( $summary['pipeline_value'] ); ?></strong>
                        </div>
                    </div>

                    <?php if ( empty( $summary['stage_breakdown'] ) ) : ?>
                        <div class="crm-empty">
                    ...
                    <?php else : ?>
                        <?php foreach ( $summary['stage_breakdown'] as $stage ) :
                            $pct = $summary['pipeline_value'] > 0
                                ? round( ( $stage->stage_value / $summary['pipeline_value'] ) * 100 )
                                : 0;
                        ?>
    ...
    <style>
    .crm-pipeline-summary-grid {
        display: grid;
        grid-template-columns: 1fr;
        gap: 20px;
    }
    .crm-pipeline-summary-card { min-height: 100%; }
    .crm-pipeline-summary-head {
        display: flex;
        align-items: flex-start;
        justify-content: space-between;
        gap: 12px;
    ...
    .crm-pipeline-summary-metrics {
        display: flex;
        gap: 12px;
        flex-wrap: wrap;
        margin-bottom: 16px;
    }
    .crm-pipeline-summary-metric {
        flex: 1 1 140px;
        background: #f9fafb;
        border: 1px solid #e5e7eb;
        border-radius: 10px;
    ...
    .crm-pipeline-summary-metric span {
        display: block;
        font-size: 12px;
        color: #6b7280;
        margin-bottom: 4px;
    }
    .crm-pipeline-summary-metric strong {
        font-size: 15px;
        color: #111827;
    }
...
function crm_deals_tab( $business_id, $detail_record_id = 0 ) {
    ...
    $active_pipeline_id = $is_detail_view && ! empty( $detail_record['pipeline_id'] )
        ? (int) $detail_record['pipeline_id']
        : crm_get_active_pipeline_id();

    $users = get_users( [ 'fields' => [ 'ID', 'display_name' ] ] );

    $stages = crm_get_pipeline_stages_for_business( $business_id, $active_pipeline_id );
    $products = crm_get_pipeline_products_for_business( $business_id, $active_pipeline_id );
    $all_products = crm_get_all_pipeline_products_for_business( $business_id );
    $pipeline_defaults = crm_get_pipeline_default_ids_for_business( $business_id );
    $default_product_id = $pipeline_defaults[ $active_pipeline_id ]['product_id'] ?? 0;
    $default_stage_id   = $pipeline_defaults[ $active_pipeline_id ]['stage_id'] ?? 0;
    $pipeline_name      = crm_get_pipeline_name( $active_pipeline_id );
    $grouped_products = [];
    foreach ( crm_get_fixed_pipelines() as $pipeline ) {
        $grouped_products[ $pipeline['id'] ] = [];
    }
    foreach ( $all_products as $product ) {
        $grouped_products[ (int) $product->pipeline_id ][] = $product;
    }
    ...
    <?php echo crm_render_pipeline_switcher( 'deals', $active_pipeline_id ); ?>

    <?php if ( $is_detail_view ) : ?>
        <?php echo crm_render_record_detail_page(
            'deal',
            $detail_record,
            crm_get_pipeline_tab_url( 'deals', $active_pipeline_id ),
            'Edit Deal'
        ); ?>
    <?php endif; ?>
    ...
    <div class="crm-filter-row" style="justify-content:space-between;">
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;flex:1;">
            ...
            <select id="crm-deal-filter-product">
        ...
        <div class="crm-modal">
            ...
            <div class="crm-modal-body">
                ...
                <div class="crm-field-group">
                    ...
                    <select id="crm-deal-product">
                        <option value="">Select product...</option>
                        <?php foreach ( crm_get_fixed_pipelines() as $pipeline ) : ?>
                            <optgroup label="<?php echo esc_attr( $pipeline['name'] ); ?>">
                                <?php foreach ( $grouped_products[ $pipeline['id'] ] ?? [] as $product ) : ?>
                                    <option value="<?php echo intval( $product->id ); ?>">
                                        <?php echo esc_html( $product->name ); ?>
                                    </option>
                                <?php endforeach; ?>
                    ...
                    <div style="margin-top:6px;font-size:12px;color:#6b7280;">
                        Pipeline:
                        <strong id="crm-deal-pipeline-label"><?php echo esc_html( $pipeline_name ); ?></strong>
                    </div>
                </div>
    ...
    (function() {
        ...
        var currentPipelineId = <?php echo intval( $active_pipeline_id ); ?>;
        var page        = 1;
        var perPage     = 20;
        var totalPages  = 1;
        var searchTimer = null;
        ...
        var detailRecord = <?php echo wp_json_encode( $detail_record ); ?>;
        var initialLoadStarted = false;
        var stagesData  = window.crm_pipeline_stages || [];
        var productsData = window.crm_pipeline_products || [];
        var pipelineDefaults = window.crm_pipeline_defaults || {};
        var pipelineCatalog = window.crm_pipelines || [];
        var productPipelineLabel = document.getElementById('crm-deal-pipeline-label');
        window.crm_active_pipeline_id = currentPipelineId;

        function getPipelineName(pipelineId) {
            pipelineId = parseInt(pipelineId || 0, 10);
            for (var i = 0; i < pipelineCatalog.length; i++) {
                if (parseInt(pipelineCatalog[i].id, 10) === pipelineId) {
                    return pipelineCatalog[i].name || 'Pipeline';
                }
            }
            return 'Pipeline';
        }
        ...
        function getProductById(productId) {
            ...
            return productsData.find(function(p) { return parseInt(p.id, 10) === productId; }) || null;
        }

        function getStagesForPipeline(pipelineId) {
            pipelineId = parseInt(pipelineId || 0, 10);
            return stagesData.filter(function(stage) {
                return parseInt(stage.pipeline_id, 10) === pipelineId;
            });
        }

        function getProductsForPipeline(pipelineId) {
            pipelineId = parseInt(pipelineId || 0, 10);
            return productsData.filter(function(product) {
                return parseInt(product.pipeline_id, 10) === pipelineId;
            });
        }

        function getDefaultPipelineProductId(pipelineId) {
            pipelineId = parseInt(pipelineId || 0, 10);
            return (pipelineDefaults[pipelineId] && pipelineDefaults[pipelineId].product_id) ? parseInt(pipelineDefaults[pipelineId].product_id, 10) : 0;
        }

        function getDefaultPipelineStageId(pipelineId) {
            pipelineId = parseInt(pipelineId || 0, 10);
            return (pipelineDefaults[pipelineId] && pipelineDefaults[pipelineId].stage_id) ? parseInt(pipelineDefaults[pipelineId].stage_id, 10) : 0;
        }

        function getCurrentPipelineStages() {
            return getStagesForPipeline(currentPipelineId);
        }

        function getCurrentPipelineProducts() {
            return getProductsForPipeline(currentPipelineId);
        }

        function getDealQueryFilters(isBoardView) {
            return {
                search:   document.getElementById('crm-deal-search').value,
                stage_id: document.getElementById('crm-deal-filter-stage').value,
                product_id: document.getElementById('crm-deal-filter-product').value,
                status:   isBoardView ? 'open' : document.getElementById('crm-deal-filter-status').value,
                owner_id: document.getElementById('crm-deal-filter-owner').value,
                pipeline_id: currentPipelineId
            };
        }

        function renderStageOptions(pipelineId, selectedStageId) {
            var stageSelect = document.getElementById('crm-deal-stage');
            if (!stageSelect) return;

            var stages = getStagesForPipeline(pipelineId);
            var html = '<option value="">Select stage...</option>';
            stages.forEach(function(stage) {
                var selected = parseInt(stage.id, 10) === parseInt(selectedStageId || 0, 10) ? ' selected' : '';
                html += '<option value="' + stage.id + '"' + selected + '>' + crmEsc(stage.name || 'Stage') + '</option>';
            });
            stageSelect.innerHTML = html;

            if (stages.length > 0 && !stageSelect.value) {
                stageSelect.value = stages[0].id;
            }
        }

        function syncProductPipeline(productId, selectedStageId) {
            var product = getProductById(productId);
            var pipelineId = product ? parseInt(product.pipeline_id, 10) : currentPipelineId;
            var pipelineLabel = product ? getPipelineName(pipelineId) : getPipelineName(currentPipelineId);
            if (productPipelineLabel) {
                productPipelineLabel.textContent = pipelineLabel;
            }
            renderStageOptions(pipelineId, selectedStageId);
            return pipelineId;
        }

        function setDealDefaults() {
            var productSelect = document.getElementById('crm-deal-product');
            var defaultProductId = getDefaultPipelineProductId(currentPipelineId);
            if (productSelect) {
                productSelect.value = defaultProductId || '';
            }
            if (defaultProductId) {
                syncProductPipeline(defaultProductId, getDefaultPipelineStageId(currentPipelineId));
            } else {
                renderStageOptions(currentPipelineId, getDefaultPipelineStageId(currentPipelineId));
            }
        }

        var dealProductSelect = document.getElementById('crm-deal-product');
        if (dealProductSelect) {
            dealProductSelect.addEventListener('change', function() {
                var productId = this.value;
                var product = getProductById(productId);
                var pipelineId = product ? parseInt(product.pipeline_id, 10) : currentPipelineId;
                var currentStageId = document.getElementById('crm-deal-stage').value;
                var pipelineStages = getStagesForPipeline(pipelineId);
                var stageIsValid = pipelineStages.some(function(stage) {
                    return parseInt(stage.id, 10) === parseInt(currentStageId || 0, 10);
                });
                var nextStageId = stageIsValid ? currentStageId : getDefaultPipelineStageId(pipelineId);
                syncProductPipeline(productId, nextStageId);
            });
        }
        ...
        function loadDealsList() {
            ...
            var filters = getDealQueryFilters(false);
        ...
        function renderDealsTable(deals) {
            var wrap = document.getElementById('crm-deals-table-wrap');
            var pipelineStages = getCurrentPipelineStages();
            ...
            var html = '<div class="bntm-table-wrapper" style="border:none;border-radius:0;">'
                + '<table class="bntm-table"><thead><tr>'
                + '<th>Deal</th><th>Amount</th><th>Product</th><th>Stage</th>'
            ...
            deals.forEach(function(d) {
                var stObj     = pipelineStages.find(function(s){ return s.id == d.stage_id; });
                ...
                html += '<tr>'
                    ...
                    + '<td>' + ( d.product_name ? crmEsc(d.product_name) : '—' ) + '</td>'
                    + '<td>'
        ...
        function loadDealsBoard() {
            ...
            var filters = getDealQueryFilters(true);
            crmPost( 'crm_get_deals', {
                search: filters.search,
                stage_id: filters.stage_id,
                product_id: filters.product_id,
        ...
        function renderBoard(deals) {
            ...
            pipelineStages.forEach(function(stage) {
                ...
                html += '<div class="crm-deal-card" draggable="true" '
                    ...
                    if ( d.product_name ) {
                        html += '<div class="crm-deal-card-meta" style="margin-top:4px;color:#6b7280;">'
                            + crmEsc(d.product_name) + '</div>';
                    }
                    if ( d.contact_name ) {
        ...
        function resetDealModal() {
            ...
            document.getElementById('crm-deal-product').value = '';
            ...
            if (productPipelineLabel) {
                productPipelineLabel.textContent = getPipelineName(currentPipelineId);
            }
        ...
        function populateDealModal(d) {
            ...
            var selectedProductId = d.product_id || getDefaultPipelineProductId(currentPipelineId);
            document.getElementById('crm-deal-product').value    = selectedProductId || '';
            syncProductPipeline(selectedProductId, d.stage_id || getDefaultPipelineStageId(currentPipelineId));
        ...
        document.getElementById('crm-deal-save-btn').addEventListener('click', function() {
            ...
            var productId = document.getElementById('crm-deal-product').value;
            ...
            if ( ! productId ) { crmToast('Please select a product.', 'error'); return; }
            ...
            crmPost( action, {
                ...
                if (isDetailView) {
                    var targetPipeline = parseInt(data.pipeline_id || currentPipelineId, 10);
                    if (targetPipeline && targetPipeline !== currentPipelineId) {
                        window.location.href = crmBuildUrl({ tab: 'deals', pipeline: targetPipeline, view: 'deal', id: dealId || data.deal_id || 0 });
                        return;
                    }
                    window.location.reload();
                    return;
        ...
        ['crm-deal-filter-stage','crm-deal-filter-product','crm-deal-filter-status','crm-deal-filter-owner'].forEach(function(id) {
...
function crm_settings_tab( $business_id ) {
    ...
    $active_pipeline_id = crm_get_active_pipeline_id();
    $active_pipeline    = crm_get_pipeline( $active_pipeline_id );
    ...
    <?php echo crm_render_pipeline_switcher( 'settings', $active_pipeline_id ); ?>
        ...
        <div class="crm-settings-col">
            ...
            <div class="bntm-form-section">
                <div style="display:flex;align-items:center;justify-content:space-between;
                            margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid #f3f4f6;">
                    <div>
                        <h3 style="margin:0;padding:0;border:none;">Products</h3>
                        <p style="font-size:13px;color:#6b7280;margin:4px 0 0;">
                            Manage products for <?php echo esc_html( $active_pipeline['name'] ); ?>.
                        </p>
                    </div>
                    <button class="bntm-btn-primary bntm-btn-small crm-settings-cta" id="crm-product-add-btn">
                        <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M12 4v16m8-8H4"/>
                        </svg>
                ...
                <ul class="crm-product-list" id="crm-product-sortable">
                    <?php if ( empty( $products ) ) : ?>
                        <li id="crm-product-empty-msg"
                            style="text-align:center;padding:24px;color:#9ca3af;font-size:13px;list-style:none;">
                            No products yet. Add your first pipeline product.
                        </li>
                    <?php else : ?>
                        <?php foreach ( $products as $product ) : ?>
                        <li class="crm-product-item"
                            data-id="<?php echo intval( $product->id ); ?>"
                            draggable="true">
                            <span class="crm-product-drag-handle">
                                <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                          d="M4 8h16M4 16h16"/>
                                </svg>
                            </span>
                            <span class="crm-product-name">
                                <?php echo esc_html( $product->name ); ?>
                            </span>
                            <button class="bntm-btn-secondary bntm-btn-small crm-product-edit-btn"
                                    data-id="<?php echo intval( $product->id ); ?>"
                                    data-name="<?php echo esc_attr( $product->name ); ?>">
                                Edit
                            </button>
                            <button class="bntm-btn-danger bntm-btn-small crm-product-delete-btn"
                                    data-id="<?php echo intval( $product->id ); ?>"
                                    data-name="<?php echo esc_attr( $product->name ); ?>">
                                Delete
                            </button>
    ...
    <div class="crm-modal-overlay" id="crm-product-modal">
        <div class="crm-modal" style="max-width:420px;">
            <div class="crm-modal-header">
                <h3 data-default="Add Product">Add Product</h3>
                <button class="crm-modal-close" onclick="crmCloseModal('crm-product-modal')">
                    <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M6 18L18 6M6 6l12 12"/>
                    </svg>
            ...
            <div class="crm-modal-body">
                <input type="hidden" id="crm-product-id" value="">
                <div class="crm-field-group">
                    <label>Product Name <span style="color:#ef4444;">*</span></label>
                    <input type="text" id="crm-product-name" placeholder="e.g. Custom Web Dev">
                </div>
                <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;
                            padding:12px;margin-top:4px;">
                    <p style="font-size:12px;color:#6b7280;margin:0;">
                        Products are unique to this pipeline. Deleting a product is blocked while it is still linked to deals.
                    </p>
                </div>
            </div>
            <div class="crm-modal-footer">
                <button class="bntm-btn-secondary" onclick="crmCloseModal('crm-product-modal')">
                    Cancel
                </button>
                <button class="bntm-btn-primary" id="crm-product-save-btn">Save Product</button>
            </div>
        </div>
    </div>
    ...
    <script>
    (function() {
        var activePipelineId = <?php echo intval( $active_pipeline_id ); ?>;
        window.crm_active_pipeline_id = activePipelineId;
        ...
        var productList  = document.getElementById('crm-product-sortable');
        ...
        function createProductItem(id, name) {
            var li = document.createElement('li');
            li.className = 'crm-product-item';
            li.dataset.id = id;
            li.draggable = true;
            li.innerHTML =
                '<span class="crm-product-drag-handle">'
                ...
                + '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8h16M4 16h16"/>'
                + '</svg></span>'
                + '<span class="crm-product-name">' + crmEsc(name) + '</span>'
                + '<button class="bntm-btn-secondary bntm-btn-small crm-product-edit-btn" '
                + 'data-id="' + id + '" data-name="' + crmEsc(name) + '">Edit</button>'
                + '<button class="bntm-btn-danger bntm-btn-small crm-product-delete-btn" '
                + 'data-id="' + id + '" data-name="' + crmEsc(name) + '">Delete</button>';
            return li;
        }
        ...
        function saveProductOrder() {
            ...
            var ids = [];
            productList.querySelectorAll('.crm-product-item[data-id]').forEach(function(li) {
                ids.push( li.dataset.id );
            });
            if ( ids.length === 0 ) return;
            crmPost( 'crm_reorder_products', { product_ids: ids.join(',') }, function() {
                crmToast('Product order saved.', 'success');
            });
        }

        if (productList) {
            productList.addEventListener('dragstart', function(e) {
                var li = e.target.closest('.crm-product-item');
                if ( ! li ) return;
                draggedProduct = li;
                li.style.opacity = '.45';
                e.dataTransfer.effectAllowed = 'move';
            ...
            productList.addEventListener('dragend', function(e) {
                var li = e.target.closest('.crm-product-item');
                if ( li ) li.style.opacity = '1';
                document.querySelectorAll('.crm-product-item').forEach(function(el) {
                    el.classList.remove('drag-over');
                });
                draggedProduct = null;
                saveProductOrder();
            ...
            productList.addEventListener('dragover', function(e) {
                e.preventDefault();
                var li = e.target.closest('.crm-product-item');
                if ( ! li || li === draggedProduct ) return;
                document.querySelectorAll('.crm-product-item').forEach(function(el) {
                    el.classList.remove('drag-over');
                });
                li.classList.add('drag-over');
                ...
                var midpoint = rect.top + rect.height / 2;
                if ( e.clientY < midpoint ) {
                    productList.insertBefore( draggedProduct, li );
                } else {
                    productList.insertBefore( draggedProduct, li.nextSibling );
                }
            });

            productList.addEventListener('dragleave', function(e) {
                var li = e.target.closest('.crm-product-item');
                if ( li ) li.classList.remove('drag-over');
            });
        }

        document.getElementById('crm-product-add-btn').addEventListener('click', function() {
            document.getElementById('crm-product-id').value = '';
            document.getElementById('crm-product-name').value = '';
            document.querySelector('#crm-product-modal .crm-modal-header h3').textContent = 'Add Product';
            crmOpenModal('crm-product-modal');
        });

        document.addEventListener('click', function(e) {
            var btn = e.target.closest('.crm-product-edit-btn');
            if ( ! btn ) return;
            document.getElementById('crm-product-id').value   = btn.dataset.id || '';
            document.getElementById('crm-product-name').value = btn.dataset.name || '';
            document.querySelector('#crm-product-modal .crm-modal-header h3').textContent = 'Edit Product';
            crmOpenModal('crm-product-modal');
        });

        document.addEventListener('click', function(e) {
            var btn = e.target.closest('.crm-product-delete-btn');
            if ( ! btn ) return;
            crmConfirm(
                'Delete product "' + btn.dataset.name + '"? This will be blocked if the product is still linked to deals.',
                function() {
                    crmPost( 'crm_delete_product', { product_id: btn.dataset.id }, function(data) {
                        crmToast(data.message || 'Product deleted.', 'success');
                        var li = productList ? productList.querySelector('.crm-product-item[data-id="' + btn.dataset.id + '"]') : null;
                        if ( li ) {
                            li.style.transition = 'opacity .3s';
                            li.style.opacity    = '0';
                            setTimeout(function() {
                                li.remove();
                                if ( productList && ! productList.querySelector('.crm-product-item') ) {
                                    var empty = document.getElementById('crm-product-empty-msg');
                                    if ( ! empty ) {
                                        empty = document.createElement('li');
                                        empty.id = 'crm-product-empty-msg';
                                        empty.style.textAlign = 'center';
                                        empty.style.padding = '24px';
                                        empty.style.color = '#9ca3af';
                                        empty.style.fontSize = '13px';
                                        empty.style.listStyle = 'none';
                                        empty.textContent = 'No products yet. Add your first pipeline product.';
        ...
        document.getElementById('crm-product-save-btn').addEventListener('click', function() {
            var btn       = this;
            var productId = document.getElementById('crm-product-id').value;
            var name      = document.getElementById('crm-product-name').value.trim();

            if ( ! name ) {
                crmToast('Product name is required.', 'error');
                return;
            }
            ...
            crmPost( 'crm_save_product', {
                product_id: productId,
                name: name
            }, function(data) {
                crmToast(data.message || 'Product saved.', 'success');
                crmCloseModal('crm-product-modal');
                btn.disabled   = false;
                btn.textContent = 'Save Product';

                var newId = data.product_id || productId;
                var li = productList ? productList.querySelector('.crm-product-item[data-id="' + productId + '"]') : null;
                if ( li ) {
                    li.querySelector('.crm-product-name').textContent = name;
                    var editBtn = li.querySelector('.crm-product-edit-btn');
                    var deleteBtn = li.querySelector('.crm-product-delete-btn');
                    if ( editBtn ) {
                        editBtn.dataset.name = name;
                    ...
                    productList.appendChild( createProductItem( newId, name ) );
                }
            }, function() {
                btn.disabled   = false;
                btn.textContent = 'Save Product';
...
function bntm_ajax_crm_get_dashboard_stats() {
    ...
    $pipeline_breakdowns = [];
    foreach ( crm_get_fixed_pipelines() as $pipeline_id => $pipeline ) {
        $pipeline_breakdowns[ $pipeline_id ] = crm_get_pipeline_dashboard_summary_for_business( $business_id, $pipeline_id );
    }
    ...
    wp_send_json_success( [
        ...
        'stage_breakdown'     => $pipeline_breakdowns[ crm_get_default_pipeline_id() ]['stage_breakdown'] ?? [],
        'pipeline_breakdowns' => $pipeline_breakdowns,
    ] );
}
    ...
    switch ( $object_type ) {
        case 'contact':
            ...
            $record['related_deals'] = $wpdb->get_results( $wpdb->prepare(
                "SELECT d.id, d.name, d.amount, d.status, d.pipeline_id, ps.name AS stage_name, p.name AS product_name
                 FROM {$wpdb->prefix}crm_deals d
                 LEFT JOIN {$wpdb->prefix}crm_pipeline_stages ps ON ps.id = d.stage_id
                 LEFT JOIN {$wpdb->prefix}crm_products p ON p.id = d.product_id
        ...
        case 'deal':
            $record = $wpdb->get_row( $wpdb->prepare(
                "SELECT d.*,
                        ...
                        p.pipeline_id AS product_pipeline_id,
...
function crm_get_detail_url( $tab, $view, $record_id = 0 ) {
    return crm_get_pipeline_tab_url( $tab, crm_get_active_pipeline_id(), $view, $record_id );
}

function crm_get_tab_url( $tab ) {
    return crm_get_pipeline_tab_url( $tab, crm_get_active_pipeline_id() );
}

function crm_render_pipeline_switcher( $tab, $active_pipeline_id, $view = '', $record_id = 0 ) {
    $pipelines = crm_get_fixed_pipelines();
    ob_start();
    ?>
    <div class="crm-pipeline-switcher">
        <span class="crm-pipeline-switcher-label">Pipeline</span>
        <div class="crm-pipeline-switcher-list">
            <?php foreach ( $pipelines as $pipeline ) :
                $is_active = (int) $active_pipeline_id === (int) $pipeline['id'];
            ?>
                <a class="crm-pipeline-switcher-item <?php echo $is_active ? 'active' : ''; ?>"
                   href="<?php echo crm_get_pipeline_tab_url( $tab, $pipeline['id'] ); ?>">
...
function bntm_ajax_crm_get_deals() {
    ...
    global $wpdb;
    $business_id = crm_get_current_business_scope_id();
    $pipeline_id  = crm_get_active_pipeline_id();
    $search      = sanitize_text_field( $_POST['search']   ?? '' );
    ...
    $where   = [
        $wpdb->prepare( 'd.business_id = %d', $business_id ),
        $wpdb->prepare( 'd.pipeline_id = %d', $pipeline_id ),
    ];
    ...
    if ( $product_id > 0 ) {
        $where[] = $wpdb->prepare( 'd.product_id = %d', $product_id );
    }
    if ( $owner_id > 0 ) {
...
function bntm_ajax_crm_create_deal() {
    ...
    if ( $product_id <= 0 ) {
        wp_send_json_error( [ 'message' => 'A product is required.' ] );
    }
    if ( $stage_id <= 0 ) {
    ...
    $product = $wpdb->get_row( $wpdb->prepare(
        "SELECT id, name, pipeline_id
         FROM {$wpdb->prefix}crm_products
         WHERE id = %d AND business_id = %d AND status = 'active'",
        $product_id, $business_id
    ) );
    if ( ! $product ) {
        wp_send_json_error( [ 'message' => 'Selected product does not exist.' ] );
    }
    ...
    if ( (int) $stage->pipeline_id !== (int) $product->pipeline_id ) {
        wp_send_json_error( [ 'message' => 'Selected stage does not belong to the selected product pipeline.' ] );
    }
    ...
    $pipeline_name = crm_get_pipeline_name( $product->pipeline_id );
    ...
    crm_log_activity(
        ...
        $deal_id,
        'note',
        'Deal created: ' . $name . ' — Pipeline: ' . $pipeline_name . ', Product: ' . $product->name . ', Stage: ' . $stage->name
    );
...
function bntm_ajax_crm_update_deal() {
    ...
    $existing = $wpdb->get_row( $wpdb->prepare(
        "SELECT id, stage_id, product_id, pipeline_id, name FROM {$wpdb->prefix}crm_deals
         WHERE id = %d AND business_id = %d",
        $deal_id, $business_id
    ) );
    ...
    $product_changed = ( (int) $existing->product_id !== $product_id );
    $pipeline_changed = ( (int) $existing->pipeline_id !== (int) $product->pipeline_id );
    ...
    if ( $stage_changed || $product_changed || $pipeline_changed ) {
        $old_stage_name = $wpdb->get_var( $wpdb->prepare(
            "SELECT name FROM {$wpdb->prefix}crm_pipeline_stages WHERE id = %d AND business_id = %d",
            $existing->stage_id, $business_id
        ) );
        $activity_body  = 'Deal moved from "' . $old_stage_name . '" to "' . $stage->name . '"';
        $activity_type  = 'deal_moved';
    }

    if ( $product_changed || $pipeline_changed ) {
        $activity_body .= ' — Product: ' . $product->name . ', Pipeline: ' . crm_get_pipeline_name( $product->pipeline_id );
    }
...
function bntm_ajax_crm_move_deal_stage() {
    ...
    $deal = $wpdb->get_row( $wpdb->prepare(
        "SELECT id, name, stage_id, pipeline_id FROM {$wpdb->prefix}crm_deals
         WHERE id = %d AND business_id = %d",
        $deal_id, $business_id
    ) );
    ...
    $stage = $wpdb->get_row( $wpdb->prepare(
        "SELECT id, name, pipeline_id FROM {$wpdb->prefix}crm_pipeline_stages
         WHERE id = %d AND business_id = %d AND status = 'active'",
        $stage_id, $business_id
    ) );
    ...
    if ( (int) $deal->pipeline_id !== (int) $stage->pipeline_id ) {
        wp_send_json_error( [ 'message' => 'This stage belongs to a different pipeline.' ] );
    }
    ...
    if ( $stage_id > 0 ) {
        $exists = (int) $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}crm_pipeline_stages
             WHERE id = %d AND business_id = %d AND pipeline_id = %d",
            $stage_id, $business_id, $pipeline_id
        ) );
        ...
        $updated = $wpdb->update(
            ...
            [ 'id' => $stage_id, 'business_id' => $business_id, 'pipeline_id' => $pipeline_id ],
            [ '%s', '%s' ],
            [ '%d', '%d', '%d' ]
        );
...
function bntm_ajax_crm_reorder_stages() {
    ...
    $stage_count = (int) $wpdb->get_var( $wpdb->prepare(
        "SELECT COUNT(*) FROM {$wpdb->prefix}crm_pipeline_stages
         WHERE business_id = %d AND pipeline_id = %d AND status = 'active'
           AND id IN ({$placeholders})",
        array_merge( [ $business_id, $pipeline_id ], $ids )
    ) );

    if ( $stage_count !== count( $ids ) ) {
        wp_send_json_error( [ 'message' => 'One or more stages do not belong to the selected pipeline.' ] );
    }
    ...
    foreach ( $ids as $index => $id ) {
        $updated = $wpdb->update(
            $wpdb->prefix . 'crm_pipeline_stages',
            [ 'sort_order' => $index + 1 ],
            [ 'id' => $id, 'business_id' => $business_id, 'pipeline_id' => $pipeline_id ],
            [ '%d' ],
            [ '%d', '%d', '%d' ]
        );
...
function bntm_ajax_crm_save_product() {
    check_ajax_referer( 'crm_nonce', 'nonce' );
    ...
    if ( $product_id > 0 ) {
        $exists = (int) $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}crm_products
             WHERE id = %d AND business_id = %d AND pipeline_id = %d AND status = 'active'",
            $product_id, $business_id, $pipeline_id
        ) );
        ...
        $duplicate = (int) $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}crm_products
             WHERE business_id = %d AND pipeline_id = %d AND status = 'active'
               AND name = %s AND id != %d",
            $business_id, $pipeline_id, $name, $product_id
        ) );

        if ( $duplicate > 0 ) {
            wp_send_json_error( [ 'message' => 'A product with this name already exists.' ] );
        }

        $updated = $wpdb->update(
            $wpdb->prefix . 'crm_products',
            [ 'name' => $name ],
            [ 'id' => $product_id, 'business_id' => $business_id, 'pipeline_id' => $pipeline_id ],
            [ '%s' ],
            [ '%d', '%d', '%d' ]
        );

        if ( $updated === false ) {
            wp_send_json_error( [ 'message' => 'Failed to update product. Please try again.' ] );
        }
    ...
    $duplicate = (int) $wpdb->get_var( $wpdb->prepare(
        "SELECT COUNT(*) FROM {$wpdb->prefix}crm_products
         WHERE business_id = %d AND pipeline_id = %d AND status = 'active' AND name = %s",
        $business_id, $pipeline_id, $name
    ) );
    ...
    if ( ! $inserted ) {
        wp_send_json_error( [ 'message' => 'Failed to create product. Please try again.' ] );
    }
...
function bntm_ajax_crm_delete_product() {
    check_ajax_referer( 'crm_nonce', 'nonce' );
    ...
    $active_product_count = (int) $wpdb->get_var( $wpdb->prepare(
        "SELECT COUNT(*) FROM {$wpdb->prefix}crm_products
         WHERE business_id = %d AND pipeline_id = %d AND status = 'active'",
        $business_id, $pipeline_id
    ) );
    ...
    if ( $active_product_count <= 1 ) {
        wp_send_json_error( [ 'message' => 'You must have at least one product in a pipeline. Add another product before deleting this one.' ] );
    }

    $linked_deals = (int) $wpdb->get_var( $wpdb->prepare(
        "SELECT COUNT(*) FROM {$wpdb->prefix}crm_deals
         WHERE product_id = %d AND business_id = %d AND status != 'deleted'",
        $product_id, $business_id
    ) );

    if ( $linked_deals > 0 ) {
        wp_send_json_error( [
            'message' => 'Cannot delete product with ' . $linked_deals . ' linked deal(s). Reassign those deals first.',
        ] );
    }
    ...
    if ( $deleted === false ) {
        wp_send_json_error( [ 'message' => 'Failed to delete product. Please try again.' ] );
    }

    wp_send_json_success( [ 'message' => 'Product deleted successfully.' ] );
}

function bntm_ajax_crm_reorder_products() {
    check_ajax_referer( 'crm_nonce', 'nonce' );
    ...
    if ( $product_ids === '' ) {
        wp_send_json_error( [ 'message' => 'No product order provided.' ] );
    }
    ...
    if ( empty( $ids ) ) {
        wp_send_json_error( [ 'message' => 'Invalid product order data.' ] );
    }
    ...
    if ( $product_count !== count( $ids ) ) {
        wp_send_json_error( [ 'message' => 'One or more products do not belong to the selected pipeline.' ] );
    }
    ...
    foreach ( $ids as $index => $id ) {
        ...
        if ( $updated === false ) {
            $wpdb->query( 'ROLLBACK' );
            wp_send_json_error( [ 'message' => 'Failed to save product order. Please try again.' ] );
        }
    }

    $wpdb->query( 'COMMIT' );

    wp_send_json_success( [ 'message' => 'Product order saved.' ] );
}
