    ...
    <div class="bntm-stats-row">
        <div class="bntm-stat-card">
            <div class="stat-icon" style="background:var(--bntm-primary,#6366f1);">
                <svg width="24" height="24" fill="none" stroke="white" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4" stroke-width="2"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>
            </div>
            <div class="stat-content">
                <h3>Total Contacts</h3>
                <p class="stat-number"><?php echo number_format($stats['total_contacts']); ?></p>
                <span class="stat-label"><?php echo number_format($stats['active_contacts']); ?> active</span>
            </div>
        </div>
        <div class="bntm-stat-card">
            ...
            <div class="stat-content">
                <h3>Open Leads</h3>
                <p class="stat-number"><?php echo number_format($stats['open_leads']); ?></p>
                <span class="stat-label"><?php echo crm_format_price($stats['pipeline_value']); ?> pipeline</span>
            </div>
        </div>
        <div class="bntm-stat-card">
        ...
        <div class="bntm-stat-card">
            <div class="stat-icon" style="background:#3b82f6;">
                <svg width="24" height="24" fill="none" stroke="white" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg>
            </div>
            <div class="stat-content">
                <h3>Interactions</h3>
                <p class="stat-number"><?php echo number_format($stats['total_interactions']); ?></p>
                <span class="stat-label">this month: <?php echo number_format($stats['monthly_interactions']); ?></span>
            </div>
        </div>
    </div>
...
function crm_leads_tab($business_id) {
    ...
    $stages = crm_get_pipeline_stages($business_id);
    ...
    foreach ($stages as $stage) {
        $leads_by_stage[$stage] = $wpdb->get_results($wpdb->prepare(
            "SELECT l.*, c.first_name, c.last_name
             ...
             WHERE l.business_id = %d AND l.stage = %s AND l.status = 'open'
    ...
    <div id="lead-view-pipeline">
        <div class="crm-pipeline">
            ...
            <div class="crm-pipeline-col">
                ...
                <div class="crm-lead-card" onclick="crmEditLead(<?php echo esc_attr(json_encode(['rand_id'=>$l->rand_id,'title'=>$l->title,'contact_id'=>$l->contact_id,'value'=>$l->value,'stage'=>$l->stage,'priority'=>$l->priority,'expected_close'=>$l->expected_close,'notes'=>$l->notes,'status'=>$l->status])); ?>)">
    ...
    <div id="lead-view-list" style="display:none;">
        ...
        <div class="bntm-table-wrapper">
            <table class="bntm-table">
                <thead><tr><th>Title</th><th>Contact</th><th>Value</th><th>Stage</th><th>Priority</th><th>Close Date</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                ...
                <tr>
                    ...
                    <td><span class="crm-badge crm-badge-<?php echo esc_attr($l->stage); ?>"><?php echo esc_html($l->stage); ?></span></td>
                    ...
                    <td>
                        <div class="crm-actions">
                            <button class="bntm-btn-icon" title="Edit" onclick="crmEditLead(<?php echo esc_attr(json_encode(['rand_id'=>$l->rand_id,'title'=>$l->title,'contact_id'=>$l->contact_id,'value'=>$l->value,'stage'=>$l->stage,'priority'=>$l->priority,'expected_close'=>$l->expected_close,'notes'=>$l->notes,'status'=>$l->status])); ?>)">
                                <svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </button>
            ...
            <div class="crm-modal-body">
                ...
                <div class="crm-form-row triple">
                    <div class="crm-form-group"><label>Stage</label>
                        <select id="add-lead-stage" class="crm-select">
                            <?php foreach ($stages as $s): ?><option value="<?php echo esc_attr($s); ?>"><?php echo esc_html($s); ?></option><?php endforeach; ?>
                        </select>
                    </div>
    ...
    <div class="crm-modal-overlay" id="edit-lead-modal">
        <div class="crm-modal">
            ...
            <div class="crm-modal-body">
                ...
                <div class="crm-form-row triple">
                    <div class="crm-form-group"><label>Stage</label>
                        <select id="edit-lead-stage" class="crm-select">
                            <?php foreach ($stages as $s): ?><option value="<?php echo esc_attr($s); ?>"><?php echo esc_html($s); ?></option><?php endforeach; ?>
                        </select>
                    </div>
                    <div class="crm-form-group"><label>Priority</label>
...
function bntm_ajax_crm_edit_lead() {
    ...
    $result = $wpdb->update(
        $wpdb->prefix . 'crm_leads',
        ['title'=>$title,'contact_id'=>$contact_id,'value'=>$value,'stage'=>$stage,'priority'=>$priority,'expected_close'=>$close_date,'status'=>$status,'notes'=>$notes],
        ['rand_id'=>$rand_id,'business_id'=>$business_id],
        ['%s','%d','%f','%s','%s','%s','%s','%s'],
        ['%s','%d']
    );
...
function crm_get_pipeline_stages($business_id) {
    $saved = bntm_get_setting('crm_pipeline_stages_' . $business_id, '');
    if ($saved) {
...
function crm_get_stats($business_id) {
    ...
    $open_leads      = (int) $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$leads_table} WHERE business_id = %d AND status = 'open'", $business_id));
